  ...
  return (
    <Layout>
      <Section>
        ...
        <LinkButton href={VOLUNTEER_SIGN_UP_LINK} onClick={(e) => openNewTab(e, VOLUNTEER_SIGN_UP_LINK)}>Sign Up Now</LinkButton>
      </Section>
      ...
      <Section>
        ...
        <Grid>
          {events.map((event) => (
            <Link key={event.id} href='/event/[id]' as={`/event/${event.id}`} passHref>
              <BaseLink>
                <Card title={new Date(event.datetime).toLocaleDateString()}>
                  <p>{event.name}</p>
                  <p>{`${event.volunteers.length} ${event.volunteers.length === 1 ? 'volunteer' : 'volunteers'}`}</p>
                </Card>
              </BaseLink>
            </Link>
          ))}
...
export async function getStaticProps(): Promise<GetStaticPropsResult<HomePageProps>> {
  ...
  try {
    const eventRecords = await getTable('Events').select({
